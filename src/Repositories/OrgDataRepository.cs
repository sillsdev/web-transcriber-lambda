...
namespace SIL.Transcriber.Repositories
{
    public class OrgDataRepository : BaseRepository<OrgData>
    {
        ...
        private async Task<IQueryable<OrgData>> GetData(IQueryable<OrgData> entities, string start)
        {
            ...
            do
            {
                if (!CheckAdd(0, dbContext.Activitystates, dtBail, jsonApiSerializer,  iStart, ref iStartNext, ref data)) break;
                if (!CheckAdd(1, dbContext.Projecttypes,dtBail, jsonApiSerializer, iStart, ref iStartNext, ref data)) break;
                if (!CheckAdd(2, dbContext.Plantypes,dtBail, jsonApiSerializer, iStart, ref iStartNext, ref data)) break;
                if (!CheckAdd(3, dbContext.Roles,dtBail, jsonApiSerializer, iStart, ref iStartNext, ref data)) break;
                if (!CheckAdd(4, dbContext.Integrations,dtBail, jsonApiSerializer, iStart, ref iStartNext, ref data)) break;
                if (!CheckAdd(5, dbContext.Plantypes,dtBail, jsonApiSerializer, iStart, ref iStartNext, ref data)) break;

                IEnumerable<Organization> orgs = await organizationService.GetAsync(); //this limits to current user
                if (!CheckAdd(6, orgs,dtBail, jsonApiSerializer, iStart, ref iStartNext, ref data)) break;
                IEnumerable<GroupMembership> gms = await gmService.GetAsync(); ;
                if (!CheckAdd(7, gms,dtBail, jsonApiSerializer, iStart, ref iStartNext, ref data)) break;
                //invitations
                if (!CheckAdd(8, dbContext.Invitations.Join(orgs, i => i.OrganizationId, o => o.Id, (i, o) => i),dtBail, jsonApiSerializer, iStart, ref iStartNext, ref data)) break;
                //groups
                if (!CheckAdd(9, dbContext.Groups.Join(gms, g => g.Id, gm => gm.GroupId, (g, gm) => g),dtBail, jsonApiSerializer, iStart, ref iStartNext, ref data)) break;
                //orgmems
                ...
                if (!CheckAdd(10, oms,dtBail, jsonApiSerializer, iStart,  ref iStartNext, ref data)) break;
                //users
                if (!CheckAdd(11, oms.Count() > 0 ? 
                                            dbContext.Users.Join(oms, u => u.Id, om => om.UserId, (u, om) => u).Where(x => !x.Archived) : 
                                            dbContext.Users.Where(x => x.Id == CurrentUser.Id),dtBail, jsonApiSerializer, iStart, ref iStartNext, ref data)) break;

                //projects
                IEnumerable<Project> projects = dbContext.Projects.Join(gms, p => p.GroupId, gm => gm.GroupId, (p, gm) => p).Where(x => !x.Archived);
                if (!CheckAdd(12, projects,dtBail, jsonApiSerializer, iStart, ref iStartNext, ref data)) break;
                //projectintegrations
                if (!CheckAdd(13, dbContext.Projectintegrations.Join(projects, pl => pl.ProjectId, p => p.Id, (pl, p) => pl).Where(x => !x.Archived),dtBail, jsonApiSerializer, iStart, ref iStartNext, ref data)) break;
                iStartNext = -1; //Done!
